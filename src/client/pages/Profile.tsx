...
interface ProfileData {
  profile: { name: string; stats: PlayerStats | null; games: GameHistoryEntry[] }
}
...
export function Profile({ name }: { name: string }) {
  ...
  const [stats, setStats] = useState<PlayerStats | null>(matchesSeed ? seeded!.stats : null)
  const [games, setGames] = useState<GameHistoryEntry[] | null>(matchesSeed ? seeded!.games : null)
  ...
  useEffect(() => {
    ...
    setLoading(true)
    Promise.all([
      api.playerStats(name).then((r) => r.stats).catch(() => null),
      api.playerGames(name).then((r) => r.games).catch(() => []),
    ]).then(([s, g]) => {
      setStats(s)
      setGames(g)
      setLoading(false)
  ...
  if (!stats) return <div className={styles.page}>{t('profile_not_found')}</div>

  const resultFor = (g: GameHistoryEntry): 'win' | 'loss' | 'draw' => {
    if (g.result === 'draw') return 'draw'
    const myColorWhite = g.whiteName === name
    const iWon = (g.result === 'w' && myColorWhite) || (g.result === 'b' && !myColorWhite)
  ...
  return (
    <div className={styles.page}>
      ...
      <section className={styles.statsGrid}>
        ...
        <Stat value={`${Math.round(stats.winRate * 100)}%`} label={t('ranking_winrate')} />
            ...
            return (
              <li key={g.id} className={styles.gameRow}>
                <span className={`${styles.badge} ${styles[r]}`}>
                  {t(r === 'win' ? 'profile_result_win' : r === 'loss' ? 'profile_result_loss' : 'profile_result_draw')}
                </span>
                <span className={styles.vs}>
                  {t('profile_vs')} <strong>{opponent}</strong>
                </span>
                <span className={styles.tc}>{TIME_CONTROLS[g.timeControl].label}</span>
                <span className={styles.moves}>{g.movesCount} ♟</span>
                <span className={styles.date}>{new Date(g.endedAt).toLocaleDateString()}</span>
              </li>
            )
          })}
        </ul>
