...
export function Lobby() {
  ...
  useEffect(() => {
    ...
    const refresh = () => socket.emit('lobby:list', (res) => res.ok && setRooms(res.data))
    refresh()
    if (socket.connected) refresh()
    socket.on('connect', refresh)

    const onAdded = (r: RoomSummary) => setRooms((prev) => [r, ...prev.filter((x) => x.id !== r.id)])
  ...
  return (
    <div className={styles.page}>
            ...
            <li key={room.id} className={styles.card}>
              <div className={styles.cardMain}>
                ...
                <span className={styles.meta}>
                  <span className={styles.badge}>{TIME_CONTROLS[room.timeControl].label}</span>
                  <span className={styles.players}>{t('lobby_players', { n: room.players })}</span>
                </span>
              </div>
      ...
      {joinTarget && (
        <div className={styles.overlay} onClick={() => setJoinTarget(null)}>
          ...
          >
            ...
            />
            {joinError && <p className={styles.joinErr}>{t(joinError as never)}</p>}
      ...
      {joinError && !joinTarget && <p className={styles.joinErr}>{t(joinError as never)}</p>}
    </div>
  )
}
