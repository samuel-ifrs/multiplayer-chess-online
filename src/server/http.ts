...
import { composeHtml, loadInitialData, type InitialState, type RenderFn } from './ssr'
...
async function mountDev(app: Express): Promise<void> {
  ...
  app.use('*', async (req, res, next) => {
    ...
    try {
      const templateRaw = fs.readFileSync(path.resolve(rootDir, 'index.html'), 'utf-8')
      const template = await vite.transformIndexHtml(url, templateRaw)
      const { render } = (await vite.ssrLoadModule('/src/client/entry-server.tsx')) as {
        render: RenderFn
      }
...
async function mountProd(app: Express): Promise<void> {
  ...
  const { render } = (await import(
    path.resolve(rootDir, 'dist/server-entry/entry-server.js')
  )) as { render: RenderFn }
  ...
  app.use('*', async (req, res, next) => {
    try {
      const html = await renderPage(render, template, req.headers.cookie, req.originalUrl)
      res.status(200).set({ 'Content-Type': 'text/html' }).end(html)
    } catch (err) {
      next(err)
    }
