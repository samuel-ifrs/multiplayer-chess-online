...
const respondDrawSchema = z.object({ roomId: z.string().min(1).max(16), accept: z.boolean() })

export function registerGameHandlers(io: ChessServer, socket: ChessSocket): void {
  ...
  socket.on('game:move', (payload, ack) => {
    ...
    const color = room.colorOf(user.id)
    if (!color) return ack({ ok: false, error: 'not_a_player' })
    if (room.turn() !== color) return ack({ ok: false, error: 'not_your_turn' })
