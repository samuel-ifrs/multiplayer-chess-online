...
import { broadcastLobbyUpdate, broadcastLobbyRemoved, startTicker } from './lifecycle'
...
export function registerLobbyHandlers(io: ChessServer, socket: ChessSocket): void {
  ...
  socket.on('lobby:join', async (payload, ack) => {
    ...
    if (!existing) {
      ...
      if (room.isPrivate) {
        const ok = room.passwordHash && password ? await verifyPassword(password, room.passwordHash) : false
        if (!ok) {
    ...
    if (room.bothPresent() && room.status === 'waiting') {
      ...
      for (const color of ['w', 'b'] as const) {
        ...
        if (sid) io.to(sid).emit('game:start', room.snapshotFor(room.players[color]!.userId))
      }
